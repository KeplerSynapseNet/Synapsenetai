...
CurlFetchResult curlFetch(const std::string& url, const CurlFetchOptions& options) {
    ...
    std::string body;
    body.reserve(std::min<size_t>(options.maxBytes, 256 * 1024));
    char buf[4096];
    while (true) {
        size_t n = fread(buf, 1, sizeof(buf), fp);
        if (n == 0) break;
        size_t take = n;
        if (body.size() + take > options.maxBytes) {
            take = options.maxBytes - body.size();
        }
        if (take > 0) body.append(buf, take);
        if (body.size() >= options.maxBytes) break;
    }
