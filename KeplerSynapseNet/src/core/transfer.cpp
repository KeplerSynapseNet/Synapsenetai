...
static bool isValidNgtAddress(const std::string& address) {
    if (address.size() != 56) return false;
    if (address.rfind("ngt1", 0) != 0) return false;
...
static bool verifyTransactionLocked(
    ...
    for (const auto& inp : tx.inputs) {
        ...
        std::string ownerAddr = addressFromPubKey(inp.pubKey);
        if (ownerAddr.empty() || !isValidNgtAddress(ownerAddr)) return false;
    ...
    for (const auto& outp : tx.outputs) {
        ...
        if (!isValidNgtAddress(outp.address)) return false;
...
static bool verifyTransactionForBlock(
    ...
    for (const auto& inp : tx.inputs) {
        ...
        if (createdIt != createdOutpoints.end()) {
            ...
            if (createdIt->second.address != ownerAddr) return false;
            amount = createdIt->second.amount;
            createdIt->second.spent = true;
        } else {
            auto it = utxoSet.find(ownerAddr);
            if (it == utxoSet.end()) return false;
            bool found = false;
            for (const auto& utxo : it->second) {
                if (!utxo.spent && utxo.txHash == inp.prevTxHash && utxo.outputIndex == inp.outputIndex) {
                    amount = utxo.amount;
...
bool TransferManager::applyBlockTransactionsFromBlock(
    ...
    for (const auto& incoming : txs) {
        ...
        for (const auto& inp : tx.inputs) {
            ...
            std::string ownerAddr = addressFromPubKey(inp.pubKey);
            if (ownerAddr.empty() || !isValidNgtAddress(ownerAddr)) {
                rollback();
                return false;
            }
        ...
        for (uint32_t i = 0; i < tx.outputs.size(); ++i) {
            const auto& outp = tx.outputs[i];
            if (outp.amount == 0 || !isValidNgtAddress(outp.address)) {
                rollback();
                return false;
            }
