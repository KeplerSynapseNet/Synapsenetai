...
struct DashboardScreen::Impl {
    ...
    void drawSparkline(int y, int x, int w, const std::vector<double>& data, int color);
...
void DashboardScreen::Impl::drawSparkline(int y, int x, int w, 
                                           const std::vector<double>& data, int color) {
    ...
    for (size_t i = 0; i < data.size() && static_cast<int>(i) < w; i++) {
        int level = static_cast<int>((data[i] / maxVal) * (numBlocks - 1));
...
void DashboardScreen::Impl::drawBalancePanel() {
    ...
    wattron(win, A_BOLD);
    mvwprintw(win, row++, panelX + 4, "%s", formatAmount(wallet.balance).c_str());
    wattroff(win, A_BOLD);
    
    row++;
    mvwprintw(win, row++, panelX + 2, "Pending:  %s", formatAmount(wallet.pending).c_str());
    mvwprintw(win, row++, panelX + 2, "Staked:   %s", formatAmount(wallet.staked).c_str());
    
    row++;
    double total = wallet.balance + wallet.pending + wallet.staked;
    wattron(win, A_BOLD);
    mvwprintw(win, row++, panelX + 2, "Total:    %s", formatAmount(total).c_str());
    wattroff(win, A_BOLD);
}
...
void DashboardScreen::Impl::drawStatsPanel() {
    ...
    mvwprintw(win, row++, panelX + 2, "Today:    %s", formatAmount(model.earningsToday).c_str());
    mvwprintw(win, row++, panelX + 2, "This Week: %s", formatAmount(model.earningsWeek).c_str());
    mvwprintw(win, row++, panelX + 2, "Total:    %s", formatAmount(model.earningsTotal).c_str());
...
void DashboardScreen::Impl::drawActivityPanel() {
    ...
    for (size_t i = 0; i < contributions.size() && static_cast<int>(i) < maxRows; i++) {
        ...
        wattron(win, COLOR_PAIR(1));
        mvwprintw(win, row, panelX + 36, "+%.4f", c.reward);
        wattroff(win, COLOR_PAIR(1));
...
void DashboardScreen::Impl::drawKnowledgePanel() {
    ...
    // Sparkline for recent rewards
    std::vector<double> hist = network.rewardHistory;
    if (!hist.empty()) {
        drawSparkline(panelY + panelH - 2, panelX + 2, panelW - 4, hist, 1);
    }
}
