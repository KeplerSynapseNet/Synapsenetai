...
struct TUI::Impl {
    ...
    int chatScrollOffset = 0;
	    ...
	    int webPromptStep = 0;
	    int webPromptBridgeMode = 0; // 0=no-bridges/default, 1=obfs4 external 9150, 2=external Tor 9150 running
	    ...
	    int siteAllowlistTarget = 0; // 0=clearnet, 1=onion
...
void TUI::Impl::drawDashboard() {
        ...
        "  Auto-start: ON  Manual knowledge add: [K] then [C]");
    {
        ...
        bal << std::fixed << std::setprecision(8) << state.wallet.balance;
        printClippedLine(innerRow++, boxX + 2, boxW - 4,
...
void TUI::Impl::drawAttachedAgent() {
    ...
    if (attachedAgentPageIndex == 0) {
        ...
        pending << std::fixed << std::setprecision(8) << state.wallet.pending;
        line("Address: " + (state.wallet.address.empty() ? std::string("not available") : state.wallet.address));
        line("Balance: " + bal.str() + " NGT | Pending: " + pending.str() + " NGT");
        std::ostringstream earned;
        earned << std::fixed << std::setprecision(8) << state.wallet.totalEarned;
        line("Total earned: " + earned.str() + " NGT");
        ...
        for (int i = static_cast<int>(rewards.size()) - 1; i >= 0; --i) {
            ...
            rewardLine << formatUnixTime(rewards[static_cast<size_t>(i)].timestamp) << "  +"
                       << std::fixed << std::setprecision(8) << rewards[static_cast<size_t>(i)].amount
        ...
        if (ch != ERR) {
            ...
            if (!suppressGlobalQuickKeys()) {
                if (ch == 'p' || ch == 'P') {
                    ...
                    impl_->attachedAgentLogFilter = 1; // tor
