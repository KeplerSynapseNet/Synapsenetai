...
struct NetworkInfo {
    ...
    // Tor / proxy / routing diagnostics
    double torHealth = 0.0; // 0.0..1.0
    std::string proxyStatus; // e.g., "connected", "offline"
    std::string routeMode; // e.g., "clearnet", "tor", "proxy"
    // Consensus / sync diagnostics
    uint64_t consensusHeight = 0;
    uint64_t consensusLag = 0; // seconds behind
    double syncRate = 0.0; // blocks per second
    // PoE metrics
    uint64_t poeTotal = 0;
...
// Mining small API for updating PoE status in the mining screen
namespace synapse {
namespace tui {
namespace mining {
